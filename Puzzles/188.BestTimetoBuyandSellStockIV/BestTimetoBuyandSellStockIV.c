...
/***********************************************************************/
/* GetP: Return the priority of a buffer!                              */
/*                                                                     */
/***********************************************************************/
int GetP(void *buf)
{
...
/***********************************************************************/
int maxProfit(int k, int* prices, int pSize)
{
    int i, len, mprofit = 0, profit;
    void *stack = NULL, *pq = NULL;
    struct StockTrade st = {0, 0}, st2;
    int *arr;

    /* Handle corner case. */
    k = (pSize / 2 < k) ? pSize / 2 : k;

    /* Validate and allocate space */
    if (k < 1 || !prices || pSize < 1 ||
        !(stack = StackAlloc((pSize / 2) + 1,
          sizeof(struct StockTrade))) ||
        !(pq = PQAlloc(k, sizeof(int), GetP)))
        goto __exit;


    /* Seek all the possible trades */
    while (st.buyd < pSize - 1)
    {
        /* Seek the buying valley */
        while (st.buyd < pSize - 1 && prices[st.buyd] >=
               prices[st.buyd + 1])
            ++st.buyd;

        /* Seek the selling peak */
        st.selld = st.buyd + 1;
        while (st.selld < pSize - 1 && prices[st.selld + 1] >
               prices[st.selld])
            ++st.selld;

        /* If the peak is valid, then proceed */
        if (st.selld < pSize) {
            /* If the buying price of the trades in the stack is greater than
            the new one, then push those into the Q. */
            while ((!StackPeek(stack, &st2)) &&
                   (prices[st2.buyd] > prices[st.buyd]))
            {
                profit = prices[st2.selld] - prices[st2.buyd];
                if (AddToPQ(pq, &profit))
                    goto __exit;
                StackPop(stack, &st2);
            }

            /* While there is a possibility to merge, attempt the same. */
            while ((!StackPeek(stack, &st2)) &&
                   (prices[st2.selld] < prices[st.selld]))
            {
                profit = prices[st2.selld] - prices[st.buyd];
                if (AddToPQ(pq, &profit))
                    goto __exit;
                StackPop(stack, &st2);
                st.buyd = st2.buyd;
            }
            if (StackPush(stack, &st))
                goto __exit;
            st.buyd = st.selld + 1;
        }
    }

    /* Pop any remaining transactions */
    while (!StackPop(stack, &st2))
    {
        profit = prices[st2.selld] - prices[st2.buyd];
        if (AddToPQ(pq, &profit))
            goto __exit;
