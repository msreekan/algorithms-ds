                     ...
                     int **sol, int *cs, int *scount, struct stack *s)
{
    ...
    while(i < alen)
    {
        ...
        if (gen_combinations(a, i + 1, alen, psum, target, sol, cs, scount, s))
            return -1;
...
int main()
{
    ...
    int len = sizeof(a) / sizeof(int), t = 8, retSize, i;
    int a1[] = {8, 7, 4, 3};
    ...
    prn_arr(a, len);
    printf("Target = %d\n", t);
    s = combinationSum2(a, len, t, &alen, &retSize);
    if (s)
    {
    ...
    prn_arr(a1, len1);
    printf("Target = %d\n", t1);
    s = combinationSum2(a1, len1, t1, &alen, &retSize);
    if (s)
    {
        /* Loop through all the entries printing each column */
        for (i = 0; i < retSize; ++i)
